...
def execute_agent(agent, use_glei, render_mode):
    ...
    # Set up the agent
    if agent == "Sarsa":
        agent = SarsaAgent(env=env, encode_fct=mc_tile_encoding)

    elif agent == "QLearning":
        agent = QLearningAgent(env=env, encode_fct=mc_tile_encoding)
    else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train and evaluate a reinforcement learning agent on MountainCar.")
