...
class TDAgent(ABC):
    ...
    def choose_action(
        ...
        Args:
            - state (hashable): The current state.
            - time_step (str): Used to decide how to choose the action during the algorithm.
                - If this is the current state, we follow an epsilon greedy policy
                - If this is for the next state with which we will compute the q-value update, we follow a policy derived from q.
                - For example, for qlearning, for time_step t+1, we chose the argmax. For sarsa, we choose with a softmax.
    ...
    def train(
        ...
        for episode in range(nb_episodes):
            ...
            while not (task_completed or episode_over):
                ...
                action = self.choose_action(state=state, time_step="t", epsilon=epsilon)
                ...
                if not (task_completed or episode_over):
                    self.update_parameters(
                        state, action, reward, next_state, alpha, gamma, epsilon
                    )
        ...
        for episode in range(nb_episodes):
            ...
            while not (task_completed or episode_over):
                state, reward, task_completed, episode_over, _ = env.step(
                    self.choose_action(state=state, time_step="t", epsilon=0)
                )
                step += 1
                episode_reward += reward
