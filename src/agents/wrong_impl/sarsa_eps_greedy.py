...
class SarsaEpsGreedy(TDAgent):
    ...
    def choose_action(self, state, time_step, epsilon=None):
        ...
        if time_step == "t+1":
            return self.epsilon_greedy_policy(state, epsilon=epsilon)
    ...
    def update_parameters(
        ...
        q_next = self.q_value(
            next_state, self.choose_action(next_state, time_step="t+1", epsilon=epsilon)
        )
